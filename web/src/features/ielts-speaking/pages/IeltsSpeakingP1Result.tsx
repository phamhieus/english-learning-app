...
import { saveIeltsP1Session } from '../../../services/storage';
        ...
        <div className="px-4 pb-4 pt-1 border-t border-slate-200/60 dark:border-slate-700/60 space-y-4">
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-1.5">
                {qr.detectedIssues.map((issue, i) => (
                  <span key={i} className="px-2 py-0.5 rounded-full text-xs bg-amber-100 dark:bg-amber-900/20 text-amber-700 dark:text-amber-300">
                    {issue}
                  </span>
                ))}
              </div>
          ...
          <div>
            <p className="text-xs font-bold text-indigo-600 dark:text-indigo-400 uppercase tracking-wider mb-1.5">
              <Sparkles className="w-3.5 h-3.5 inline mr-1" />Reference Answer
            </p>
          ...
          {qr.pronunciationWords.length > 0 && (
            <div>
              ...
              <div className="flex flex-wrap gap-1.5">
                {qr.pronunciationWords.map((w, i) => (
                  <span key={i} className="px-2 py-0.5 rounded-full text-xs bg-purple-50 dark:bg-purple-950/20 text-purple-700 dark:text-purple-300 border border-purple-200 dark:border-purple-800/40 font-medium">
                    {w}
                  </span>
                ))}
              </div>
...
const IeltsSpeakingP1Result = () => {
    ...
    const run = async () => {
      try {
        ...
        addHistory({
          ...
          score: Math.round(res.estimatedBand * 10),
          focus: 'Speaking',
        });
      } catch (e) {
  ...
  const { criteria, questionResults } = result;
    ...
    <div className="max-w-3xl mx-auto pb-12 animate-in fade-in slide-in-from-bottom-4 duration-400">
      ...
      <div className="text-center mb-8">
        ...
        <p className="text-sm text-slate-400">
          {result.questionCount} questions · {result.topicCount} topic{result.topicCount !== 1 ? 's' : ''} · {mins}:{String(secs).padStart(2, '0')}
        </p>
      </div>

      {/* ── Estimated band ────────────────────────────────────── */}
      <div className={cn('rounded-3xl border p-6 text-center mb-6 shadow-sm', bandBg(result.estimatedBand))}>
        <p className="text-xs font-bold uppercase tracking-widest text-slate-500 mb-1">Estimated Band</p>
        <div className={cn('text-7xl font-black tabular-nums mb-1', bandColor(result.estimatedBand))}>
          {result.estimatedBand.toFixed(1)}
        </div>
        <p className="text-xs text-slate-500 dark:text-slate-400 mt-2">
          This is an AI-generated estimate for practice. It is not an official IELTS score.
        </p>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <button
          onClick={() => navigate('/speaking/ielts-p1', { state: { selectedTopic: undefined } })}
