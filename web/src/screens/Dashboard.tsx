...
const Dashboard = () => {
  ...
  return (
    <div className="animate-in fade-in duration-300">
      {/* Hero Section */}
      <section className="mb-9 relative overflow-hidden rounded-3xl bg-gradient-to-br from-indigo-500 via-purple-500 to-indigo-600 p-8 text-white shadow-xl shadow-indigo-500/20">
        <div className="relative z-10 max-w-xl">
          <p className="text-indigo-100 font-semibold tracking-widest uppercase text-xs mb-3">TOEIC · IELTS Prep</p>
          <h1 className="text-4xl font-bold mb-4 tracking-tight">Let's get you exam-ready.</h1>
      ...
      <div className="grid grid-cols-3 gap-6 mb-10">
        {[
          ...
          <div key={i} className="glass-card rounded-2xl p-6 flex items-center gap-4 hover:-translate-y-1 transition-transform">
            <div className={cn(
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 gap-6 mb-10">
        {[
          ...
          <button
            ...
            className={cn(
              'rounded-2xl p-6 flex items-center gap-4 text-left transition-all hover:-translate-y-1',
          ...
          >
            <div className={cn('w-14 h-14 rounded-2xl flex items-center justify-center text-white shadow-lg shrink-0', it.color)}>
              <it.icon className="w-7 h-7" />
            </div>
            <div>
              <span className="font-bold text-slate-800 dark:text-slate-200 block">{it.title}</span>
              <span className="text-sm text-slate-400 dark:text-slate-500 font-medium">{it.sub}</span>
            </div>
          </button>
        ))}
      </div>
